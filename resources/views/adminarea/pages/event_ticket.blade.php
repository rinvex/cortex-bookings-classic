...
@push('inline-scripts')
    {!! JsValidator::formRequest(Cortex\Bookings\Http\Requests\Adminarea\EventTicketFormRequest::class)->selector("#adminarea-events-tickets-create-form, #adminarea-events-{$event->getRouteKey()}-tickets-{$eventTicket->getRouteKey()}-update-form") !!}
@endpush
    ...
    <div class="content-wrapper">
        ...
        <section class="content">
            ...
            <div class="nav-tabs-custom">
                @if($eventTicket->exists && $currentUser->can('delete', $eventTicket)) <div class="pull-right"><a href="#" data-toggle="modal" data-target="#delete-confirmation" data-modal-action="{{ route('adminarea.events.tickets.destroy', ['event' => $event, 'ticket' => $eventTicket]) }}" data-modal-title="{!! trans('cortex/foundation::messages.delete_confirmation_title') !!}" data-modal-body="{!! trans('cortex/foundation::messages.delete_confirmation_body', ['resource' => trans('cortex/bookings::common.event_ticket'), 'identifier' => $eventTicket->name]) !!}" title="{{ trans('cortex/foundation::common.delete') }}" class="btn btn-default" style="margin: 4px"><i class="fa fa-trash text-danger"></i></a></div> @endif
