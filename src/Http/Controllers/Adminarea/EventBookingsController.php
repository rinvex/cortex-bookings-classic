...
class EventBookingsController extends AuthorizedController
{
    ...
    public function index(Event $event, EventBookingsDataTable $eventsDataTable)
    {
        return $eventsDataTable->with([
            ...
            'id' => "adminarea-cortex-bookings-events-{$event->getRouteKey()}-bookings",
    ...
    {
        if (! $eventBooking->exists && $request->has('replicate') && $replicated = $eventBooking->resolveRouteBinding($request->input('replicate'))) {
