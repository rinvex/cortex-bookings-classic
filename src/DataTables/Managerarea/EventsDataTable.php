...
use Cortex\Bookings\Transformers\Managerarea\EventTransformer;
...
class EventsDataTable extends AbstractDataTable
{
    ...
    protected function getColumns(): array
    {
        $link = config('cortex.foundation.route.locale_prefix')
            ? '"<a href=\""+routes.route(\'managerarea.events.edit\', {event: full.id, locale: \''.$this->request->segment(1).'\'})+"\">"+data+"</a>"'
            : '"<a href=\""+routes.route(\'managerarea.events.edit\', {event: full.id})+"\">"+data+"</a>"';
